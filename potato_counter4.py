"""
================================================================================
 Techno Seeds — Conveyor Potato Counter (Laptop / OpenVINO / CPU / Live Camera)
 ALL-IN-ONE VERSION: live counting + zone calibration + worker bag review

 CPU / OpenVINO EDITION — this is the CPU-optimized counterpart to the GPU
 (CUDA) version. Use this one on any machine without an NVIDIA GPU; use the
 GPU version on the RTX 3060 machine.

 Carries forward every fix already made in the CPU line:
   1) FPS from a rolling window of real frame timestamps, not since script
      start (avoids idle STOPPED time skewing the number).
   2) Camera explicitly requests MJPG (not just the output writer) —
      avoids USB-bandwidth-driven FPS variance from raw YUY2 capture.
   3) cv2.CAP_PROP_BUFFERSIZE = 1 — avoids frame-queue lag ("slow
      detection" symptom from stale buffered frames).
   4) CPU thread pinning is opt-in via --threads N (not forced by default).
   5) Startup model-verification block: confirms TARGET_CLASS_NAME
      actually matches a class in the loaded model before you waste a
      belt test on a silent zero-detections bug.
   6) Startup reports whether the model is end2end/NMS-free (YOLO26 is) —
      if so, IOU_THRESH may be a no-op, flagged so you don't waste time
      re-tuning it blind.
   7) Optional --debug flag: logs confidence + frames-unseen pattern at
      the moment each potato is first counted.

 NEW in this version:
   8) Optional --tta flag (test-time augmentation): runs each frame through
      the model with additional flipped/scaled passes and merges results.
      This is a genuine accuracy lever (catches some detections a single
      pass misses, at the cost of roughly 2-3x slower inference). Off by
      default — turn it on only if you've confirmed CPU speed has margin
      to spare (i.e. you're not already bottlenecked near the camera's
      max FPS).
================================================================================
...
1) CALIBRATION MODE — run once to set the zone to match your belt exactly:
     python potato_counter_cpu.py --calibrate
...
# Thread pinning is OPT-IN via --threads N, not forced on by default.
# Forcing NUM_THREADS to os.cpu_count() (logical cores, e.g. 8 on a
# 4-core/8-thread CPU via hyperthreading) can oversubscribe the 4 physical
# cores and cause contention that's SLOWER than letting OpenVINO auto-tune.
# Must be parsed from sys.argv here (before other imports) since env vars
# need to be set before cv2/ultralytics load.
_threads_arg = None
...
CONF_THRESH = 0.44           # Roboflow-reported optimal confidence
IOU_THRESH = 0.55
...
# NEW: test-time augmentation — off by default, real accuracy/speed trade-off
USE_TTA = False
...
def load_zone_ratios():
    ...
    if os.path.exists(ZONE_CONFIG_PATH):
        ...
        print("[INFO] No zone_config.json found — using default zone ratios. "
              "Run 'python potato_counter_cpu.py --calibrate' to set this precisely.")
...
# ============================================================================
# Camera setup — MJPG capture format + buffer size 1
# ============================================================================
...
def verify_model(model):
    ...
    if USE_TTA:
        print("[VERIFY] Test-time augmentation (--tta) is ENABLED. Expect roughly "
              "2-3x slower inference in exchange for somewhat better recall — "
              "watch the live FPS number to make sure you still have headroom "
              "against the camera's max FPS.")
    print("-" * 60)
...
def run_counter():
    ...
    while True:
        ...
        if mode == "RUNNING":
            ...
            if FRAME_SKIP <= 1 or detect_frame_idx % FRAME_SKIP == 0:
                results = model.predict(
                    ...
                    imgsz=IMG_SIZE, device=DEVICE, half=HALF_PRECISION,
        ...
        if key == ord("q"):
            ...
            mode = "PAUSED"
            print(f"[INFO] PAUSED. Count preserved at {counter.total_count}. Press 's' to resume.")
    ...
    print(f"[RESULT] Resolution: {w}x{h}  |  Overall average FPS: {avg_fps:.1f}")
...
# ============================================================================
def parse_args():
    p = argparse.ArgumentParser(description="Techno Seeds Potato Counter — CPU/OpenVINO edition")
    ...
    p.add_argument("--threads", type=str, default=None,
                    help="Pin CPU threads, e.g. --threads 4 (physical core count)")
    p.add_argument("--tta", action="store_true",
                    help="Enable test-time augmentation for higher recall at ~2-3x slower inference")
