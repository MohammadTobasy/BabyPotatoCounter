"""
================================================================================
 Techno Seeds — Conveyor Potato Counter (GPU / CUDA / Live Camera)
 ALL-IN-ONE VERSION: live counting + zone calibration + worker bag review

 GPU EDITION — changes vs. the CPU/OpenVINO version are marked with "# GPU:"
 Summary of what changed and why:
   1) Inference now runs on an NVIDIA GPU via PyTorch/CUDA (Ultralytics'
      native .pt inference path), NOT OpenVINO. OpenVINO targets CPU/Intel
      iGPU — it is the wrong tool once a real NVIDIA GPU is available.
      Loads best26.pt directly; no OpenVINO export step needed.
   2) DEVICE now defaults to "cuda:0" instead of "cpu". A startup check
      confirms CUDA is actually available and reports the GPU name + VRAM;
      if CUDA isn't available it warns loudly and falls back to CPU rather
      than crashing, so the script is still runnable on a machine without
      a GPU attached.
   3) HALF_PRECISION (FP16) now defaults to True. RTX 3060 has full FP16
      tensor-core support — this roughly doubles throughput vs FP32 with
      negligible accuracy impact. Disable with --no-half if you ever need
      to isolate an accuracy question from a precision question.
   4) CPU thread-pinning (OMP_NUM_THREADS / --threads) has been REMOVED —
      it was a CPU-inference-only lever and does nothing useful for GPU
      inference. torch.backends.cudnn.benchmark is enabled instead, which
      is the equivalent "let the backend auto-tune for this exact input
      shape" lever for GPU (safe here because CAMERA_WIDTH/HEIGHT and
      IMG_SIZE are fixed for the whole run, so the input shape never
      changes mid-run).
   5) Startup verification block updated: reports GPU name/VRAM/CUDA
      version instead of OpenVINO compiled-model info, still checks
      TARGET_CLASS_NAME against the model's class list, and still reports
      whether the model is end2end/NMS-free (YOLO26 is, regardless of
      backend) so IOU_THRESH's relevance is still flagged correctly.
   6) Everything NOT related to inference backend is UNCHANGED on purpose:
      zone-entry counting logic, Hungarian-matching, camera handling
      (MJPG capture request + CAP_PROP_BUFFERSIZE=1), rolling-window FPS,
      Start/Pause/Resume/Finish-bag/Review/Reset controls, history
      logging, camera auto-reconnect. None of that was CPU-specific, so
      none of it needed to change for the GPU port.
================================================================================
...
# GPU: torch is imported explicitly (not just via ultralytics) so we can
# query CUDA availability / device name / VRAM before doing anything else.
import torch
...
# CONFIG
# ============================================================================
# GPU: point this at the raw ultralytics checkpoint, not an OpenVINO export
# folder. No export step is required to run on GPU.
MODEL_PATH = "best26.pt"
...
CONF_THRESH = 0.001         # Roboflow-reported optimal confidence for this model
IOU_THRESH = 0.70
IMG_SIZE = 640               # must match training/export imgsz
...
# GPU: defaults changed. DEVICE now targets the first CUDA GPU; HALF_PRECISION
# (FP16) now defaults on, since an RTX 3060 has proper tensor-core FP16
# support and this roughly doubles throughput vs FP32 with negligible
# accuracy impact. Both are overridable via --device / --no-half.
DEVICE = "cuda:0"
HALF_PRECISION = True
...
# Toggled via --debug. Logs per-object confidence at the moment each potato
# is first counted, to help correlate "late/missed/flicker" with
# low-confidence or motion-blurred frames during belt testing.
DEBUG_LOG = False

# How many recent frames to average FPS over, instead of averaging over the
# entire run (including idle STOPPED time).
FPS_WINDOW = 30
...
# ============================================================================
# Shared camera setup — MJPG capture format + buffer size 1 (unchanged from
# the CPU version — these were camera/USB fixes, not inference-backend fixes)
# ============================================================================
def open_camera(camera_index, width, height):
    ...
    # Request MJPG from the camera itself. Many USB webcams default to raw
    # YUY2 over DirectShow, which is bandwidth-limited at higher
    # resolutions/fps and can silently cap or destabilize the delivered
    # frame rate. MJPG is compressed on-camera and much less likely to
    # saturate USB bandwidth. Must be set BEFORE width/height for some
    # drivers to honor it.
    ...
    # Keep only 1 frame buffered. Without this, if processing falls behind
    # the camera's capture rate even briefly, OpenCV/DirectShow can queue up
    # stale frames — you then "catch up" by processing frames that are
    # already old, which shows up as a potato being detected late (already
    # near/past the zone) or the count lagging behind reality.
...
# ============================================================================
# GPU: model + device verification block (replaces the OpenVINO version)
# ============================================================================
def verify_model_and_device(model):
    print("-" * 60)
    ...
    # Report whether the underlying model is end-to-end / NMS-free. If True,
    # IOU_THRESH passed to model.predict() may have little or no effect on
    # the final boxes — this is true regardless of CPU vs GPU backend.
    end2end = None
    try:
    ...
    # GPU: device/CUDA verification (replaces the old OpenVINO thread-count block)
    print(f"[VERIFY] Requested DEVICE = {DEVICE}, HALF_PRECISION = {HALF_PRECISION}")
    if torch.cuda.is_available():
        ...
        total_vram_gb = torch.cuda.get_device_properties(gpu_idx).total_memory / (1024 ** 3)
        print(f"[VERIFY] CUDA available. GPU: {gpu_name} ({total_vram_gb:.1f} GB VRAM), "
              f"CUDA version: {torch.version.cuda}")
    else:
        print("[VERIFY] *** WARNING ***: CUDA is NOT available on this machine "
              "(no GPU detected, or PyTorch was installed without CUDA support). "
              "Falling back to CPU — expect much lower FPS than on the RTX 3060. "
              "Check 'pip install torch' was done with the correct CUDA index URL "
              "(see the environment setup notes) and that 'nvidia-smi' works.")
    print("-" * 60)
...
def run_counter():
    ...
    # GPU: fall back to CPU automatically if CUDA isn't actually available,
    # rather than letting model.predict() fail deep inside the main loop.
    if DEVICE.startswith("cuda") and not torch.cuda.is_available():
        print("[WARN] CUDA requested but not available — falling back to DEVICE='cpu'. "
              "Performance will be much lower than on the RTX 3060.")
        DEVICE = "cpu"

    # GPU: let cuDNN auto-tune convolution algorithms for the fixed input
    # shape used every frame (IMG_SIZE x IMG_SIZE) — the GPU-side equivalent
    # of the old CPU thread-pinning lever, and safe here because the input
    # shape never changes mid-run.
    if DEVICE.startswith("cuda"):
        torch.backends.cudnn.benchmark = True

    print(f"[INFO] Loading model: {MODEL_PATH} on device={DEVICE} half={HALF_PRECISION}")
    model = YOLO(MODEL_PATH)
    model.to(DEVICE)

    verify_model_and_device(model)
    ...
    # Rolling window of recent frame timestamps for a stable, meaningful FPS
    # readout instead of an average since script start.
    ...
    while True:
        ...
        # FPS from a rolling window of real elapsed time between the oldest
        # and newest frame in the window, not since script start.
...
# ============================================================================
def parse_args():
    p = argparse.ArgumentParser(description="Techno Seeds Potato Counter — all-in-one (GPU edition)")
    ...
    # GPU: new/changed flags
    p.add_argument("--device", type=str, default=DEVICE,
                    help="Inference device, e.g. 'cuda:0' (default) or 'cpu'. "
                         "Falls back to 'cpu' automatically if CUDA isn't available.")
    p.add_argument("--no-half", action="store_true",
                    help="Disable FP16 half-precision inference (runs FP32 instead). "
                         "Useful to isolate a precision question from an accuracy question.")
    p.add_argument("--model", type=str, default=MODEL_PATH,
                    help="Path to the model checkpoint, e.g. best26.pt")
    return p.parse_args()
